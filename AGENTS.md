...
- When asked to do something, do not assume answers to questions that you do not know for sure. Use the AskUserQuestion tool about literally anything that you think will provide you more information on how to do the job better.
- Always use the code simplifier agent at the end of a coding session if we have added a significant amount of code.
...
- Once all tests pass, run the formatters and linters and mypy. Look at the pre-commit hooks to see all the things that need to run before commit.
...
## Setup
- Backend: `cd backend && uv sync` (add `--dev` for dev dependencies)
- Frontend: `cd frontend && npm install`

## Running
- Backend: `cd backend && uv run uvicorn main:app --reload`
- Frontend: `cd frontend && npm run dev`
- Docker Compose (production): `docker compose up -d` / `docker compose down` / `docker compose logs -f`
...
## Design
- In python use `dict[str, Any]` sparingly. Always prefer designing with types. Create types and pass data using properly typed objects.
